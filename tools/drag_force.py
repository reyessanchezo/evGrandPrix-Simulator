...
def aerodynamic_drag_power(wm):
    rps = np.divide(wm, 60)
    velocity = np.multiply(np.multiply(np.multiply(G, rps), math.pi), D) # velocity of the kart in m/s
    velocity3 = np.power(velocity, 3)
    section1 = np.multiply(np.multiply(np.multiply(0.5, CD), A), RHO)

    DYNO_POWER_OUT = np.multiply(section1, velocity3) # Drag power = 0.5 * CD * A * RHO * (kart velocity)^3
